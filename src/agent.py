...
def _load_labels_for_matches(match_tasks: list[dict]) -> str:
    ...
    labels_dir = _Path(__file__).parent.parent / "lota_data" / "labels"
...
def node_call_llm(state: AgentState) -> AgentState:
    ...
    if rt.session:
        rt.session.llm_call(
            ...
            response=response or "(dry-run: 无 LLM 响应)",
            tokens_in=tokens_in,
...
def node_reflect(state: AgentState) -> AgentState:
    """结算后反思 — LLM 分析结果，提炼 alpha 因子 + 归因到订单，回写 FactorMemory。
    ...
    cross_factors_text = ""
    if rt.role and rt.role.alpha_mode:
    ...
    sp = load_system_prompt(rt.role.system_prompt_name) if rt.role and rt.role.system_prompt_name else None
    ...
    persona_text = rt.role.persona_text() if rt.role else ""
    factor_desc_text = rt.role.memory.factors.factor_desc_text() if rt.role else ""
...
```"""
    ...
    if reflection and rt.role:
        ...
        if data is None:
            rt.session.reflect_call(
                ...
                response=reflection,
                params={"error": "json_parse_failed", "settled_count": len(settled)},
            )
            return state
        ...
        # 保存反思
        rt.role.memory.reflections.add_reflection(day_date, summary)
        if key_slugs_list or noise_slugs_list:
            ...
            if rt.role.memory.reflections.reflections:
                last = rt.role.memory.reflections.reflections[-1]
                last["reflection"] = last.get("reflection", "") + slug_note
                rt.role.memory.reflections._save()

        # money_lesson
        money_lesson = data.get("money_lesson", "")
        if money_lesson and rt.role.memory.reflections.reflections:
            last = rt.role.memory.reflections.reflections[-1]
    ...
    if rt.session:
        rt.session.reflect_call(
            reflect_prompt=reflect_prompt,
            response=reflection if reflection else "(dry-run)",
            params={
                "settled_count": len(settled),
                "day_date": day_date,
                "alpha": alpha,
                "key_slugs": key_slugs_str,
                "attributed_orders": len(attr_map) if reflection else 0,
            }
        )
...
def should_call_llm(state: AgentState) -> str:
    if state.get("live") and _rt(state).provider:
        return "call_llm"
    return "parse_orders"
...
class Agent:
    ...
    def factor_review(self, end_date: str, start_date: str = "") -> dict:
        """因子结构性评估 — 由外部控制调用时机。
        ...
        try:
            ...
            self._end_session(session)
        return {"end_date": end_date, "start_date": start_date or "(auto-7d)", "user": self.user, "status": "ok"}
    ...
    def analyze_live(self, lota_id: str) -> str:
        """走地分析 — LLM 综合赛前数据+走地赔率+比分 判断方向"""
        from src.data_manager import DataManager, _get
        from src.tools import extract_odds
        from src.factor_registry import FactorRegistry
        import re

        dm = DataManager()
        ctx = dm.get_match_context(lota_id)
        match = ctx.get("match", {})
        home = match.get("home", "?")
        away = match.get("away", "?")
        ...
        slugs = ["fair-odds", "eu-odds-pinnacle", "asian-handicap-pinnacle",
                 "discrete-odds", "betfair-buysell", "over-under-crown"]
        sections = {}
        for slug in slugs:
            txt = dm.get_sections(lota_id, [slug])
            if txt.strip():
                sections[slug] = txt[:800]

        # 走地 AS_ODDS
        as_odds = _get('/matches/', {'lota_id': lota_id, 'include_odds': '1', 'odds_type': 'asia'})
        m = as_odds.get("data", {}).get("matches", [{}])[0]
        live_score = m.get("score") or "?"
        live_state = m.get("state_name", "?")
        odds_data = m.get("asia_odds_data", [])
        live_lines = []
        prev_hc = None
        for o in odds_data:
            t = o.get("handicapTime", "")[-8:]
            sc = o.get("score") or ""
            hc = o.get("handicap", "")
            h = o.get("home", "")
            a = o.get("away", "")
            if sc:
                chg = ""
                if prev_hc and hc != prev_hc:
                    chg = f" ⚠️变盘 {prev_hc}→{hc}"
                live_lines.append(f"{t} {sc} {hc} H{h}/A{a}{chg}")
                prev_hc = hc

        live_text = "\n".join(live_lines[-40:]) if live_lines else "(无走地数据)"

        # 因子库
        self._ensure_role()
        rt = _rt({"user": self.user})
        fr = FactorRegistry()
        cross_factors = fr.format_for_prompt(current_date=date.today().isoformat(), window_days=7, min_samples=3)[:2000]
        my_factors = rt.role.memory.factors.perf_text() if rt.role else ""

        # 构建 prompt
        prompt = f"""你是走地亚盘分析师。根据赛前数据+走地盘口变化+比分，判断当前走地方向。

## {home} vs {away}（{league}）
当前比分: {live_score}  状态: {live_state}

## 赛前数据 (compact-fet)
"""
        for slug, txt in sections.items():
            prompt += f"### {slug}\n{txt}\n\n"
...
分析以下问题并给出判断：
1. 赛前数据指向什么方向？触发哪些因子？
2. 走地盘口如何演变？进球后盘口降速快/慢？
3. 当前比分下，走地应该怎么下？（上盘/下盘/大球/小球/不下）
4. 置信度（高/中/低）及核心理由
...
输出简洁，直接给结论。"""
        ...
        response = rt.provider.call(prompt, [{"role": "user", "content": "分析走地方向。"}], temperature=0.3)
        return response if response else "(LLM 无响应)"
