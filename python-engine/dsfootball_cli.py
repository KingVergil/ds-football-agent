...
from src.role_registry import all_agents as _all_registry_agents


def _check_review_due(agent: Agent):
    """live 模式: 检查上次 factor_review 是否超过 7 天，超时提示"""
    sessions_dir = Path("data/sessions") / agent.user
...
def cmd_factor_review(agent: Agent, rest: list):
    """因子结构性评估 — 判断因子的市场假设是否还成立（支持 --user-notes <文本>）"""
    from src.providers.deepseek import DeepSeekProvider
    agent.set_provider(DeepSeekProvider())
    user_notes = ""
    if "--user-notes" in rest:
        idx = rest.index("--user-notes")
        if idx + 1 < len(rest):
            user_notes = rest[idx + 1]
            rest = rest[:idx] + rest[idx + 2:]
    end_date = rest[0] if rest else None
    start_date = rest[1] if len(rest) > 1 else ""
    r = agent.factor_review(end_date=end_date, start_date=start_date, user_notes=user_notes)
    print(f"✅ factor_review 完成: {r['end_date']} | 窗口起始={r['start_date']} | user_notes={r['user_notes'] or '(无)'}")
...
def _extract_runall_opts(rest: list) -> tuple:
    """从参数里提取 runall 选项，返回 (位置参数, opts)。"""
    opts = {"factor_induction_every": 1, "factor_review_every": 7, "user_notes": "", "replay_dir": ""}
    out = []
    i = 0
    while i < len(rest):
        a = rest[i]
        if a == "--factor-induction-every" and i + 1 < len(rest):
            try:
                opts["factor_induction_every"] = int(rest[i + 1])
            except ValueError:
                pass
            i += 2
            continue
        if a == "--factor-review-every" and i + 1 < len(rest):
            try:
                opts["factor_review_every"] = int(rest[i + 1])
            except ValueError:
                pass
            i += 2
            continue
        if a == "--user-notes" and i + 1 < len(rest):
            opts["user_notes"] = rest[i + 1]
            i += 2
            continue
        if a == "--replay-dir" and i + 1 < len(rest):
            opts["replay_dir"] = rest[i + 1]
            i += 2
            continue
        out.append(a)
        i += 1
...
def _write_replay_report(replay_dir, run_id, start, end, total_days, dog, opts,
                         start_capital, end_capital, trajectory, reviews):
    """写 Python 侧回放轨迹报告（对齐 JS ds_replay 的 report.json/report.md）。"""
    from datetime import datetime as _dt
    report = {
        "run_id": run_id,
        "engine": "python",
        "created_at": _dt.now().isoformat(timespec="seconds"),
        "range": {"start": start, "end": end, "days": total_days},
        "dogs": [dog],
        "user_notes": opts["user_notes"],
        "factor_induction_every": opts["factor_induction_every"],
        "factor_review_every": opts["factor_review_every"],
        "start_capital": {dog: round(start_capital, 2)},
        "end_capital": {dog: round(end_capital, 2)},
        "trajectory": trajectory,
        "factor_reviews": reviews,
    }
    ...
    rows = "\n".join(
        f"| {t['day']} | {t['capital']}({t['pnl']:+.2f}) | {t['placed']} | {t['settled']} | {t['pending']} | {t['active_factors']} |"
        for t in trajectory
    ) or "(无轨迹)"
    md = f"""# 回放报告 {run_id}（python 引擎）

- 范围：{start} ~ {end}（{total_days} 天）｜狗：{dog}
- 用户意见：{opts['user_notes'] or '(无)'}｜因子归纳：每 {opts['factor_induction_every']} 天
- 起点 → 终点：{round(start_capital, 2)} → {round(end_capital, 2)}（{round(end_capital - start_capital, 2):+.2f}）

## 轨迹

| 日期 | 余额(PnL) | 下单 | 结算 | 待定 | 活跃因子 |
|---|---|---|---|---|---|
{rows}

## 因子退役记录

{chr(10).join(f"- [{r['day']}] {r.get('status', 'ok')}" for r in reviews) if reviews else '(未触发)'}
"""
    (replay_dir / "report.md").write_text(md, encoding="utf-8")


def cmd_runall(agent: Agent, rest: list, live: bool = False, jingcai_only: bool = False):
    """按日维度回放：获取比赛→分析→结算→(反思)→因子归纳→周期性因子退役。

    选项：
      --factor-induction-every N   每隔 N 天做一次因子归纳（默认 1=逐日；0=关闭）
      --factor-review-every N      每隔 N 天做一次因子退役（默认 7）
      --user-notes <文本>          周期因子退役时注入用户调整意见
      --replay-dir <目录>          轨迹报告输出目录（默认 data/replays/replay_<start>_<end>_<ts>）
    """
    pos, opts = _extract_runall_opts(rest)
    if len(pos) < 2:
        print("用法: python dsfootball_cli.py agent <狗> runall <start> <end> [--no-live] [--jingcai] "
              "[--factor-induction-every N] [--factor-review-every N] [--user-notes <文本>] [--replay-dir <目录>]")
        return
    ...
    trajectory = []
    reviews = []
    run_id = f"replay_{start}_{end}_{_dt.now().strftime('%Y%m%d%H%M%S')}"
    replay_dir = Path(opts["replay_dir"] or f"data/replays/{run_id}")
    start_capital = agent._get_capital()
    ...
    while d <= end_d:
        ...
        # 逐日因子归纳（阶段A/B：单狗场景 scope=自己；多狗全量跑时 factor_induction 已是非alpha→alpha barrier）
        if opts["factor_induction_every"] and day_count % opts["factor_induction_every"] == 0:
            from src.factor_induction import main as induction_main
            induction_main(["--roles", agent.user])

        # 每7天触发因子结构性评估（带用户调整意见）
        if opts["factor_review_every"] and day_count % opts["factor_review_every"] == 0:
            r = agent.factor_review(end_date=day_str, user_notes=opts["user_notes"])
            reviews.append({"day": day_str, "end_date": day_str,
                            "user_notes": opts["user_notes"], "status": r.get("status", "ok")})

        # 轨迹快照
        capital = agent._get_capital()
        pending = len([o for o in agent._get_unsettled_orders() if not o.get("settled_at")])
        trajectory.append({
            "day": day_str,
            ...
            "active_factors": agent._get_active_factor_count(),
        })

        # 破产检查：资金 < 50 且无待结算订单 = 真破产
        if capital < 50:
            pending_now = [o for o in agent._get_unsettled_orders() if not o.get("settled_at")]
            if not pending_now:
                print(f"🪦 资金耗尽 (¥{capital:.0f})，game over.")
                break
            else:
                print(f"⚠️ 资金不足 ¥{capital:.0f}（{len(pending_now)} 单待结算，等待比赛结果）")

        d += timedelta(days=1)

    end_capital = agent._get_capital()
    _write_replay_report(replay_dir, run_id, start, end, total_days, agent.user, opts,
                         start_capital, end_capital, trajectory, reviews)
    print(f"\n{agent.status()}")
    print(f"\n📦 回放报告: {replay_dir}")
...
if __name__ == "__main__":
    ...
    if cmd == "dashboard":
        ...
        def _extract_match(lid, order_score=""):
            ...
            feat_path = Path(f"data/features/{lid}.json")
            if feat_path.exists():
                try:
        ...
        # 狗列表从本地角色派生（公开仓库不携带默认狗；注册表新建狗自动并入）
        agents_list = _all_registry_agents(include_extra=True)
            ...
            for o in role_orders:
                ...
                if is_parlay:
                    ...
                    for l in legs:
                        ...
                        lhit = l.get("hit")
                        if lhit is True:
                            line = f'<span style="color:#4d8a5f;font-weight:bold">{line} ✅</span>'
                        elif lhit is False:
                            line = f'<span style="color:#a89b85">{line} ❌</span>'
        ...
        print(f"✅ dashboard.html 已刷新 ({len(all_orders)} 条订单, {sum(len(v) for v in curves.values())} 个狗粮数据点)")
    ...
    if cmd == "role-sync":
        """迁移入口：注册表狗补建 Python 角色（persona.md + <狗>.json）+ 旧 persona 字段迁移。

        用法: python dsfootball_cli.py role-sync [--dry-run]
        """
        from src.role_registry import sync_from_registry
        res = sync_from_registry(dry_run="--dry-run" in sys.argv)
        print("角色迁移:")
    ...
    if action == "init":
        ...
        cmd_runall(agent, rest, live=live, jingcai_only=jingcai)
    elif action == "status":
        cmd_status(agent)
    elif action == "persona":
