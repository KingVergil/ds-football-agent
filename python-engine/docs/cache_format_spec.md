...
本文只定义**盘上格式**。数据从哪来（Lota API / 自建源）由 `Fetcher` seam 负责，
不在本规范范围内。开源仓库默认只发布 `LocalCacheSource`（读缓存目录）+ 本规范；
网络实现由用户按 `Fetcher` 协议自行提供（详见 `docs/data_manager_open_source_design.md`）。
