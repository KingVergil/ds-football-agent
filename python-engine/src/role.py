...
# 沙箱回放支持：DS_ROLES_ROOT 覆盖角色根（桥在跑沙箱时设置；缺省=线上 data/roles）
ROLES_DIR = Path(os.environ.get("DS_ROLES_ROOT") or (Path(__file__).parent.parent / "data" / "roles"))
...
class Role:
    ...
    def __init__(
        ...
        self.limits: Optional[dict] = None  # 资金约束（max_exposure_pct/truncate/max_orders/min_orders）
    ...
    def load(cls, name: str) -> "Role":
        ...
        r.limits = data.get("limits") if isinstance(data.get("limits"), dict) else None
