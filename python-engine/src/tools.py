...
CACHE_ROOT = Path(os.environ.get("DSF_DATA_DIR", Path(__file__).parent.parent / "data"))
...
def get_cached_compact_fet(lota_id: str) -> Optional[dict]:
    ...
    js_features = Path(__file__).parent.parent / "data" / "features" / f"{lota_id}.json"
...
# OP 行末尾的返奖率 "(r93.23%)" —— 归一化成 "/93.23"，与 Δ 行四段对齐
_OP_RETURN_SUFFIX = re.compile(r"\(r([\d.]+)%\)$")
...
def _split_odds_line(line: str) -> list[str]:
    ...
    # OP 行末尾 "(r93.23%)" → "/93.23"，与 Δ 行的 h/d/a/r 四段对齐；
    # 这样当段落最后一行是 OP 行（如只有开盘价、无 Δ 更新）时也能正确解析。
    odds_part = _OP_RETURN_SUFFIX.sub(r"/\1", odds_part)
