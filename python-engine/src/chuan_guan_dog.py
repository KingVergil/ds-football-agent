...
独立角色/资金/订单（默认 user="串关2狗"，落在 data/roles/串关2狗/）。
...
ROLES_ROOT = Path(__file__).parent.parent / "data" / "roles"
...
class ChuanGuanDog(Agent):
    ...
    def _select_legs_llm(self, matches: list[dict], day_date: str,
        ...
        # 组合路径思考（方案A）：选腿前先展开玩法变体、比较完整路径，再按最优路径落单
        combo_need = f"路径腿数必须等于 {need} 场（票型要求）。" if forced_tickets else \
            "路径腿数 = 票型所需腿数（如 3串1 为 3 场）。"
        combo_block = f"""## 🧩 组合路径思考（选腿前必做，思考过程必须体现）
1. **变体展开**：对候选场次，分别列出「不让球」与「让球(让球线)」两种口径下你认可的方向与赔率。
   只有同一场两种口径都成立（信号、赔率双达标）时才值得比较；不成立的口径不要硬凑。
2. **拼路径**：按目标票型拼出 **2~3 条完整串关路径**，逐条写出：
   - 腿构成（例：甲让胜 + 乙胜 + 丙平）
   - 总赔率（各腿赔率连乘）
   - 每腿信号强度与风险（是否强腿/因子支持/离散或资金方向）
   例：路径1 = 甲让胜 + 乙胜 + 丙平 @9.8；路径2 = 甲胜 + 乙让胜 + 丙平 @8.6；路径3 = ...
3. **择优**：从三个方面比较后选出最优路径：
   - ① 赔率价值：总赔率 × 命中率性价比（不是越高越好，蚊子肉与高赔冷门都排除）
   - ② 信号强度：路径中强腿（高票数/因子支持）占比越高越优
   - ③ 容错：路径是否含高风险腿（离散矛盾/深盘不降/资金背离）——存在则降权
4. **落单**：只按选定路径输出每腿 order 块（一场最多一腿，类型=胜平负，让球腿盘口=让球线，不让球腿盘口=0）；
   {combo_need} 选不出 2~3 条可比较路径，或所有路径都达不到强腿标准 → 空仓。
"""
        ticket_block = f"""## 任务
{task_line}
...
- 先完成「组合路径思考」再落单：思考过程需展开玩法变体并比较完整路径，禁止直接逐场定方向
